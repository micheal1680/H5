...
app.post("/resgiter",(req,res)=>{
    let sql = `select * from  admin where username='${req.body.username}'`;
    mydb.query(sql, (err, result)=>{
        if (!result.length ) {
			if(req.body.imgurl){
				var sql =`insert  into admin (username,password,imgurl) values ('${req.body.username}','${req.body.pwd}','${req.body.imgurl}')`
			}else{
				var sql =`insert  into admin (username,password) values ('${req.body.username}','${req.body.pwd}')`
			}
            mydb.query(sql,(err,result)=>{
