    ...
    table.on("tool(cartoontable)", function (obj) {
        ...
        if (layEvent === 'del') { //删除  根据商品id删除记录  data.id
            layer.confirm('真的删除行么', function (index) {
                // ajax请求删除记录
                $.post("http://localhost:3000/cake/del", {
                    id: data.id
                }, function (data) {
            ...
            // encodeURI 转码中文
            var params = `id=${data.id}&name=${data.name}&oldprice=${data.oldprice}&newprice=${data.newprice}&description=${data.description}&picture=${data.picture}&type=${data.type}&activity=${data.activity}`;
